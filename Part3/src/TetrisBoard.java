...
public class TetrisBoard {
		...
		if (rot > 3){
			...
			for (int row=0; row < length; row++) {
				...
				for (int col=0; col < length; col++){
					//for each board column
					//if the are we check the grid
					if (currentPiece.isFilled(rotation, row, col)==true && blockMatrix[row+gridRow][col+gridCol]==true){
		...
		//If the rotation is already in its initial form then the rotation of 270 degrees is returned - index 4
			...
			for (int row=0; row < length; row++) {
				...
				for (int col=0; col < length; col++){
					//for each board column
					//if the are we check the grid
					if (currentPiece.isFilled(rot, row, col)==true && blockMatrix[row+gridRow][col+gridCol]==true){
						//if there is a block in the position that we are trying to move then collision is true
						return true;}}}
	...
	private boolean detectOutOfBounds(TetrisPiece piece, int rot, int gridRow, int gridCol){
		...
		if (rot > 3){
			...
			for (int row=0; row < length; row++) {
				...
				//for each board column
					//if the are we check the grid
					if (currentPiece.isFilled(rotation, row, col)){
						//if there is a block in the position that we are trying to move
						
						if (row+gridRow>= rowNum || col+gridCol>= colsNum || gridCol + col< 0){
							return true;}
						}
					}
				}}
	...
	private boolean validMove(TetrisPiece piece, int rot, int gridRow, int gridCol){
		...
		//First check if it's out of bounds and them detectCollisions because detect collisions has to do with 
		//the board and if it's out of bounds the board is going to be null in these positions
